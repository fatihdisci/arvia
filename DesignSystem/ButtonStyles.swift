...
// "Cockpit Black" imza butonu: dolgu yerine 1.5pt turkuaz çerçeve,
// tracked-uppercase label. Dolgu (filled) primary buton kullanılmaz.
struct PrimaryButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .overlay(
                RoundedRectangle(cornerRadius: AppRadius.button)
                    .stroke(isEnabled ? AppColors.accentPrimary : AppColors.accentPrimary.opacity(0.4), lineWidth: 1.5)
            )
...
struct DestructiveButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(AppTypography.bodyMedium)
            .foregroundColor(AppColors.textOnCritical)
            .frame(maxWidth: .infinity)
            .frame(height: AppSpacing.minimumTapTarget + 8)
            .padding(.horizontal, AppSpacing.lg)
            .background(
